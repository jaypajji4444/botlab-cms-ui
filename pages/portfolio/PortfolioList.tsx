...
export const PortfolioList: React.FC = () => {
  ...
  const handleDuplicate = async (item: PortfolioDto) => {
    const toastId = toast.loading("Duplicating portfolio...");
    try {
      const fullItem = await portfoliosApi.getById(item.id);
      const newItem = await portfoliosApi.create({
        title: `${fullItem.title} (Copy)`,
        slug: `${fullItem.slug}-copy`,
        ...
        isIndexable: false,
        status: "draft",
        numberOfDrones: fullItem.numberOfDrones,
        showDuration: fullItem.showDuration,
      });
      toast.success("Portfolio duplicated!", { id: toastId });
      navigate(`/portfolios/edit/${newItem.id}`);
    } catch (error) {
      toast.error("Could not duplicate portfolio", { id: toastId });
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      result = result.filter(
        (p) => ((p as any).status || "draft") === statusFilter,
      );
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Showcase your projects and success stories.
          </p>
        </div>
                ...
                filtered.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        variant={
                          ((item as any).status || "draft") === "published"
                            ? "success"
                            : "neutral"
                        }
                      >
